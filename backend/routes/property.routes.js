...
// Add Property
router.post("/add", protect, sellerOnly, upload.array("images", 10), async (req, res) => {
  try {
    let imageUrls = [];
...
// Update Property
router.put("/update/:id", protect, sellerOnly, upload.array("images", 10), async (req, res) => {
  try {
...
router.get("/:id", async (req, res) => {
  try {
    const property = await Property.findById(req.params.id)
      .populate("seller", "name phone email profilePic");
...
// Update property status (seller)
router.patch("/status/:id", protect, sellerOnly, async (req, res) => {
  try {
    const property = await Property.findById(req.params.id);
    if (!property) {
      return res.status(404).json({ success: false, message: "Property not found" });
    ...
    res.json({ success: true, message: "Status updated", property });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
